...
(defun gzip-compress (input &optional (start 0) (end (length input))
  ...
  (let* ((header (gzip-header-octets level))
         ...
         (size (logand (- end start) #xFFFFFFFF))
         (out (make-octet-buffer (+ 10 (length deflated) 8))))
    (replace out header)
    (replace out deflated :start1 10)
    (let ((o (+ 10 (length deflated))))
      (setf (aref out o) (ldb (byte 8 0) crc)
            (aref out (1+ o)) (ldb (byte 8 8) crc)
            (aref out (+ o 2)) (ldb (byte 8 16) crc)
            (aref out (+ o 3)) (ldb (byte 8 24) crc)
            (aref out (+ o 4)) (ldb (byte 8 0) size)
            (aref out (+ o 5)) (ldb (byte 8 8) size)
            (aref out (+ o 6)) (ldb (byte 8 16) size)
            (aref out (+ o 7)) (ldb (byte 8 24) size)))
    out))
